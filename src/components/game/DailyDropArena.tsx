...
function parsePts(value: string | null | undefined): number {
  if (!value) return 0;
...
function DailyDropArena() {
  ...
  const duelHandle = params.duel;
  const specificMatch = params.match;
  ...
  useEffect(() => {
    ...
    setDuelChallenger({
      username: duelHandle,
      pts: params.pts ? parseInt(String(params.pts), 10) || 0 : 0,
    });
  }, [duelHandle, params.pts]);
  ...
  const challengerScoreVal = duelHandle
    ? (params.pts ? parseInt(String(params.pts), 10) || 0 : 0)
    : challengerPts
