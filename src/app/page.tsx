...
function DailyDropArena() {
  ...
  const parsedPts = parseInt(searchParams.get('pts') || '0', 10);
  const safePts = Number.isFinite(parsedPts) ? parsedPts : 0;
  ...
  const duelChallenger = duelHandle ? { username: duelHandle, pts: safePts } : null;
  ...
  const handleShareShowdown = async () => {
    ...
    const challengerName = duelChallenger.username || 'scout';
    const challengerPtsSafe = Number.isFinite(duelChallenger.pts) ? duelChallenger.pts : 0;
    const outcome = userScore > challengerPtsSafe
      ? `I defeated @${challengerName}`
      : `Close match with @${challengerName}`;
    const text = [
      '⚔️ DUEL SHOWDOWN on SportsHistoryClue!',
      outcome,
      `Me: ${userScore.toLocaleString()} PTS vs @${challengerName}: ${challengerPtsSafe.toLocaleString()} PTS`,
      ...
      <main className="min-h-screen bg-[#fafafa] flex flex-col font-mono text-xs uppercase tracking-widest text-zinc-400">
        {duelHandle && (
          <div className="bg-blue-600 text-white px-6 py-2.5 text-center text-xs font-bold tracking-wide normal-case">
            ⚔️ Duel Active: Beat @{duelChallenger?.username || duelHandle || 'scout'}&apos;s score of {(duelChallenger?.pts ?? safePts).toLocaleString()} PTS!
          </div>
        )}
  ...
  const showdownDiff = duelChallenger ? Math.abs(showdownScore - (Number.isFinite(duelChallenger.pts) ? duelChallenger.pts : 0)) : 0;

  const showdownName = duelChallenger?.username || 'scout';
  const showdownPts = duelChallenger && Number.isFinite(duelChallenger.pts) ? duelChallenger.pts : 0;
  const showdownCard = duelChallenger ? (
    <section className="bg-white border border-zinc-200 rounded-3xl p-5 md:p-6 shadow-sm">
      ...
      >
        {showdownScore > showdownPts
          ? `🏆 Victory — You outperformed @${showdownName} by ${showdownDiff.toLocaleString()} PTS!`
          : showdownScore < showdownPts
          ? `💀 Defeat — @${showdownName} edged you out by ${showdownDiff.toLocaleString()} PTS!`
      ...
      <div className="mt-4 grid grid-cols-[1fr_auto_1fr] items-center gap-2 md:gap-4">
        <div className="rounded-2xl border border-zinc-200 bg-zinc-50 p-3 md:p-4 text-center">
          <span className="mx-auto mb-2 flex h-12 w-12 items-center justify-center rounded-full bg-zinc-900 text-sm font-black text-white">
            {(showdownName.charAt(0) || '?').toUpperCase()}
          </span>
          <span className="block text-xs font-black text-zinc-900 truncate">@{showdownName}</span>
          <span className="mt-1 block font-mono text-sm font-black text-zinc-900">
            {showdownPts.toLocaleString()} PTS
          </span>
        </div>
        ...
        <div className="rounded-2xl border border-blue-600 bg-blue-50 p-3 md:p-4 text-center">
          <span className="mx-auto mb-2 flex h-12 w-12 items-center justify-center rounded-full bg-blue-600 text-sm font-black text-white">
            {(youHandle.charAt(0) || 'Y').toUpperCase()}
          </span>
